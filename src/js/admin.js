...
async function updateStatus(idx, newStatus) {
  ...
  try {
const resp = await fetch(API_URL + '/updateStatus', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ref: row.ref, status: newStatus })
    });
...
const resp = await fetch(API_URL + '/updateStatus', {
             method: 'POST',
             headers: { 'Content-Type': 'application/json' },
             body: JSON.stringify({ ref: row.ref, status: targetStatus })
         });
        if (!resp.ok) throw new Error('HTTP ' + resp.status);
        const data = await resp.json();
        if (data.status !== 'ok') throw new Error(data.message || 'Unauthorized');
...
const oldStatus = row.status;
     ...
     try {
         const resp = await fetch(API_URL + '/updateStatus', {
             method: 'POST',
             headers: { 'Content-Type': 'application/json' },
             body: JSON.stringify({ ref: row.ref, status: 'รอชำระเงิน' })
         });
         if (!resp.ok) throw new Error('HTTP ' + resp.status);
         const data = await resp.json();
         if (data.status !== 'ok') throw new Error(data.message || 'Unauthorized');
...
async function cancelBooking(idx) {
    ...
    try {
const resp = await fetch(API_URL + '/cancelBooking', {
       method: 'POST',
       headers: { 'Content-Type': 'application/json' },
...
async function updateVisitorApproval(idx, pidx, val) {
    ...
    try {
        const resp = await fetch(API_URL + '/updateVisitorApproval', {
       method: 'POST',
       headers: { 'Content-Type': 'application/json' },
       body: JSON.stringify({ ref: row.ref, visitorApproved: row.visitorApproved, extraVisitorApproved: row.extraVisitorApproved, visitorCount: row.visitorCount, total: row.total })
     });
        if (!resp.ok) throw new Error('HTTP ' + resp.status);
        const data = await resp.json();
        if (data.status !== 'ok') throw new Error(data.message || 'Unauthorized');
...
const oldStatus = row.status;
 ...
 const resp = await fetch(API_URL + '/updateStatus', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ref: row.ref, status: 'ไม่อนุมัติ' })
  });
         if (!resp.ok) throw new Error('HTTP ' + resp.status);
         const data = await resp.json();
         if (data.status !== 'ok') throw new Error(data.message || 'Unauthorized');
...
try {
    ...
    // Then update status
    const statusResp = await fetch(API_URL + '/updateStatus', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ref: row.ref, status: newStatus })
    });
