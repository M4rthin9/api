...
let rows = [];
  try {
    const data = await (await fetch(API_URL + '/getAll')).json();
    ...
    renderError('ไม่สามารถเชื่อมต่อระบบไม่้า กรุณาลองใหม่อีกครั้ง');
    return;
  }
...
async function uploadSlipViaAppsScript(file, ref) {
   ...
   const result = await (await fetch(API_URL + '/uploadSlip', {
     method: 'POST',
     headers: { 'Content-Type': 'application/json' },
     body: JSON.stringify({
       ref: ref,
...
setOverlay(true, 'กำลังบันทึกการชำระเงิน...');
   try {
     const updateResp = await (await fetch(API_URL + '/updateStatus', {
       method: 'POST',
       headers: { 'Content-Type': 'application/json' },
       body: JSON.stringify({
    ...
    if (updateResp.status !== 'ok') throw new Error(updateResp.message || 'บันทึกไม่สำเร็จ');

     // Update slip image separately via getAll would need slipImage in sheet
   } catch (err) {
